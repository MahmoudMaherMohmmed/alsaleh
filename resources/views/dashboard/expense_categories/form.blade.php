    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                @endif
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ $expense_category!=null ? __('expense_categories.actions.edit') : __('expense_categories.actions.create') }}</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    <form method="post" action="{{$expense_category!=null ? route('expense_categories.update', $expense_category->id) : route('expense_categories.store')}}" enctype="multipart/form-data" data-parsley-validate="">
                        @csrf
                        @if($expense_category!=null) @method('PUT') @endif
                        <div class="row row-sm">

                            <div class="col-12">
                                <div class="form-group">
                                    <label class="form-label">{{ __('expense_categories.attributes.title') }} <span class="tx-danger">*</span></label>
                                    <div class="example">
                                                ...
                                                <div class="tab-content">
                                                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                                        <div class="tab-pane {{$loop->first ? 'active' : ''}}" id="tab-title-{{ $localeCode }}">
                                                            <input class="form-control" name="title[{{ $localeCode }}]" placeholder="{{ __('expense_categories.attributes.title') }}" value="{{$expense_category!=null ? $expense_category->getTranslation('title', $localeCode) : old('title[$localeCode]')}}" type="text" required>
                                                        </div>
                                                    @endforeach
                                                </div>
                                            </div>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    <label class="form-label">{{ __('expense_categories.attributes.description') }}</label>
                                    <div class="example">
										<div class="panel panel-primary tabs-style-1">
                                            <div class=" tab-menu-heading">
                                                <div class="tabs-menu1">
                                                ...
                                                <div class="tab-content">
                                                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                                        <div class="tab-pane {{$loop->first ? 'active' : ''}}" id="tab-description-{{ $localeCode }}">
                                                            <textarea class="form-control" name="description[{{ $localeCode }}]" placeholder="{{ __('expense_categories.attributes.description') }}"
                                                            rows="5">{{$expense_category!=null ? $expense_category->getTranslation('description', $localeCode) : old('description[$localeCode]')}}</textarea>
                                                        </div>
                                                    @endforeach
                                                </div>
                                            </div>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    <label class="form-label">{{ __('expense_categories.attributes.type') }} <span class="tx-danger">*</span></label>
                                    <select class="form-control select2-no-search" name="type" required="">
                                        @foreach(App\Enums\ExpenseCategoryTypeEnum::options() as $key=>$value)
                                            <option value="{{$key}}" {{$expense_category!=null && $expense_category->type->value==$key ? 'selected' : ''}}> {{ $value }} </option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    <label class="form-label">{{ __('expense_categories.attributes.status') }} <span class="tx-danger">*</span></label>
                                    <select class="form-control select2-no-search" name="status" required="">
                                        @foreach(App\Enums\ExpenseCategoryStatusEnum::options() as $key=>$value)
                                            <option value="{{$key}}" {{$expense_category!=null && $expense_category->status->value==$key ? 'selected' : ''}}> {{ $value }} </option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>

                            <div class="col-12"><button class="btn btn-main-primary pd-x-20 mg-t-10" type="submit">{{ $expense_category!=null ? __('expense_categories.actions.edit') : __('expense_categories.actions.save') }}</button></div>
                        </div>
                    </form>
                </div>
            </div>
