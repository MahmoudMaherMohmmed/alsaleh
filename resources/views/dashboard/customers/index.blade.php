    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ __('customers.plural') }}</span>
            </div>
        </div>
        <div class="d-flex my-xl-auto right-content">
            @include('dashboard.customers.partials.actions.create')
        </div>
    </div>
    <!-- breadcrumb -->
@endsection
                        ...
                        <table class="table text-md-nowrap" id="example2">
                            <thead>
                                <tr>
                                    ...
                                    <th class="wd-15p border-bottom-0">{{ __('customers.attributes.created_at') }}</th>
                                    <th class="wd-15p border-bottom-0">{{ __('customers.actions.actions') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($customers as $customer)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge {{$customer->status->color()}}">{{$customer->status->trans()}}</span>
                                        </td>
                                        <td>{{$customer->created_at->diffForHumans()}}</td>
                                        <td>
                                            @include('dashboard.customers.partials.actions.show')
