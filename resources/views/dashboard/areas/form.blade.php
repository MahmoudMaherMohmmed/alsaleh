    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                @endif
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ $area!=null ? __('areas.actions.edit') : __('areas.actions.create') }}</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    <form method="post" action="{{$area!=null ? route('areas.update', $area->id) : route('areas.store')}}" enctype="multipart/form-data" data-parsley-validate="">
                        @csrf
                        @if($area!=null) @method('PUT') @endif
                        <div class="row row-sm">

                            <div class="col-12">
                                <div class="form-group">
                                    <label class="form-label">{{ __('areas.attributes.title') }} <span class="tx-danger">*</span></label>
                                    <div class="example">
                                                ...
                                                <div class="tab-content">
                                                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                                        <div class="tab-pane {{$loop->first ? 'active' : ''}}" id="tab-title-{{ $localeCode }}">
                                                            <input class="form-control" name="title[{{ $localeCode }}]" placeholder="{{ __('areas.attributes.title') }}" value="{{$area!=null ? $area->getTranslation('title', $localeCode) : old('title[$localeCode]')}}" type="text" required>
                                                        </div>
                                                    @endforeach
                                                </div>
                                            </div>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    <label class="form-label">{{ __('areas.attributes.description') }}</label>
                                    <div class="example">
										<div class="panel panel-primary tabs-style-1">
                                            <div class=" tab-menu-heading">
                                                <div class="tabs-menu1">
                                                ...
                                                <div class="tab-content">
                                                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                                        <div class="tab-pane {{$loop->first ? 'active' : ''}}" id="tab-description-{{ $localeCode }}">
                                                            <textarea class="form-control" name="description[{{ $localeCode }}]" placeholder="{{ __('areas.attributes.description') }}"
                                                            rows="5">{{$area!=null ? $area->getTranslation('description', $localeCode) : old('description[$localeCode]')}}</textarea>
                                                        </div>
                                                    @endforeach
                                                </div>
                                            </div>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    <label class="form-label">{{ __('areas.attributes.status') }} <span class="tx-danger">*</span></label>
                                    <select class="form-control select2-no-search" name="status" require="">
                                        @foreach(App\Enums\AreaStatusEnum::options() as $key=>$value)
                                            <option value="{{$key}}" {{$area!=null && $area->status->value==$key ? 'selected' : ''}}> {{ $value }} </option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>

                            <div class="col-12"><button class="btn btn-main-primary pd-x-20 mg-t-10" type="submit">{{ $area!=null ? __('areas.actions.edit') : __('areas.actions.save') }}</button></div>
                        </div>
                    </form>
                </div>
            </div>
